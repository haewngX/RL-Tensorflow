...
class DQN(object):
    def __init__(self, a_dim, s_dim, learning_rate, gamma, epsilon):
        ...
        self.selected_q_next = tf.placeholder(tf.float32, [None])

        self.memory = ReplayMemory(8000)  # 初始化记忆库
        self.learn_step_counter = 0  # 学习的步数
        self.replace_target_iter = 100  # 替换target网络的频率
        # self.selected_q_next = np.zeros((128, 1))
        # 初始化eval网络 求q(s,a)
        self.q_eval = self.build_network(self.s, "eval_net", tf.AUTO_REUSE)
        # 初始化target网络 求qtar(s',a)
        self.q_next = self.build_network(self.s_, "target_net", tf.AUTO_REUSE)
        ...
        q_target = self.r + (1. - self.d) * self.gamma * self.selected_q_next
    ...
    def learn(self):
        ...
        q_next, q_eval4next = self.sess.run(
            ...
            feed_dict={self.s_: s_,
                       self.s: s_})
        batch_index = np.arange(128, dtype=np.int32)
        max_act4next = np.argmax(q_eval4next, axis=1)  # 求argmaxQ_eval(s',a)
        selected_q_next = q_next[batch_index, max_act4next]  # 求max(Q_target(s',argmaxQ_eval(s',a))
        # 更新参数
        self.sess.run(self.train_op,feed_dict={self.s: s, self.a: a,
                                                       self.s_: s_, self.r: r, self.d: np.float32(d),self.selected_q_next:selected_q_next})
        self.learn_step_counter += 1 # 学习的步数+1

    # 构建神经网络
    def build_network(self, s, scope, reuse):
